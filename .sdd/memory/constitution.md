...
5. **Endpoints destrutivos** (`/api/history/clear`, `/api/settings/clear-database`, `/api/backups/restaurar`): exclusivos de ADMIN, exigem confirmação explícita na UI (modal destrutivo) e geram entrada imutável no log de auditoria ANTES de executar. A existência desses endpoints é uma exceção consciente ao threat model e está documentada no `spec.md` (REQ-014). **A restauração de backup** só parte de um backup **verificado**, escolhido da lista — nunca arquivo enviado —, com a palavra RESTAURAR digitada; roda fora da aplicação (`.github/workflows/restaurar.yml`), faz antes um **backup de segurança** do estado atual, verificado e guardado, recusa backup de schema diferente do de produção e troca só as **tabelas de negócio**, numa transação. **A trilha de auditoria não volta no tempo** — e registra a restauração. *(Emenda de 2026-09-13, CR Tipo D, ADR-024, TASK-116: o texto anterior citava "restore de backup" sem rota nem fluxo — a restauração pela aplicação não existia desde a TASK-082, e a cláusula nomeava o que não havia. A emenda saiu depois de a restauração estar no ar (TASK-115, #75), ensaiada sobre cópia de produção; a primeira execução real em produção espera o token de disparo da TASK-114.)*

## 4. Strict Rules — Dados, Migrações e Disaster Recovery

1. **Migrações pareadas:** toda alteração de schema tem UP em `db/migrations-pg/<timestamp>_<nome>.up.sql` e DOWN pareado `<timestamp>_<nome>.down.sql` com o mesmo prefixo. DOWN escrito ANTES de aplicar o UP. Migração sem DOWN = BLOQUEADOR. **A aplicação é feita pelo runner do projeto (`db/runner-migracoes.mjs`), que registra o que aplicou em `migracoes_aplicadas` na mesma transação da migration**; aplicar schema à mão sem registrar deixa o banco e o repositório divergentes sem sintoma. *(Emenda de 2026-09-10, CR Tipo D, ADR-021: o texto anterior mandava `db/migrations/NNNN_up_*.sql`, padrão que nenhum arquivo do projeto jamais seguiu — o Gate 2 passava porque verifica pareamento, não a convenção. A emenda saiu só depois de o runner, a adoção e o aviso do health estarem em produção.)*
2. **Teste de migração:** nenhuma migration toca produção sem antes passar, em base descartável, (a) pela **ida e volta** — UP → DOWN → UP, com o schema depois do DOWN idêntico ao de antes do UP —, verificada pela suíte para todas as migrations; e (b) quando **toca dados** (`UPDATE`, `DELETE`, `INSERT`, ou restrição que dado existente pode violar: `NOT NULL`, `UNIQUE`, `CHECK`, chave estrangeira), por um **ensaio sobre a cópia restaurada do backup mais recente** (`ensaiar` do runner, runbook §4.0.2), com o resultado registrado ao lado da migration em `<timestamp>_<nome>.ensaio.md`. Produção nunca é o primeiro banco a ver uma migration. *(Emenda de 2026-09-10, CR Tipo D, ADR-022: o texto anterior mandava testar "UP + DOWN contra uma CÓPIA do banco (`backups/` ou cópia temporária)" e nunca aplicar "direto em `keys.db` de produção" — meio da stack SQLite, vencido no go-live. E o princípio não era cumprido: nenhum DOWN tinha rodado uma vez sequer até a TASK-106, e a primeira ida e volta achou um DOWN que não restaurava. A emenda saiu depois da ida e volta (TASK-106) e do ensaio (TASK-107) estarem na suíte.)*
...
4. Histórico de transações é **imutável por design** para papéis não-ADMIN; nenhuma feature nova pode permitir edição/exclusão de transação individual. A **única** reversão em massa é a restauração de backup (§3.5): devolve o histórico ao estado de um backup verificado, inteiro — nunca linha a linha —, com o estado anterior guardado no backup de segurança. *(Emenda de 2026-09-13, CR Tipo D, ADR-024, TASK-116: a restauração reverte o histórico que esta cláusula declara imutável; sem nomeá-la aqui, a cláusula afirmaria uma imutabilidade que o sistema tem um caminho legítimo para desfazer.)*
