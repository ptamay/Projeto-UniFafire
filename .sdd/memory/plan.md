...
- [x] **TASK-096 → `<Link>` e `loading.tsx` por rota.** ✅ **FEITA em 2026-09-09 — e a medição EMENDOU o ADR.** Nove `loading.tsx` (nenhuma rota tinha), quatro superfícies de navegação convertidas para `<Link>`. **Mas o prefetch ficou DESLIGADO**, contra a prescrição original, porque o número mandou:
  ...
  > O prefetch não comprava nada: o esqueleto vem do BUNDLE DA ROTA, não do payload prefetchado. E `router.refresh()` invalida o cache e faz TODOS os links prefetcharem de novo — ele é chamado pelo `refreshData` a cada sinal do Realtime, ou seja **a cada operação de chave, em cada cliente aberto**. Multiplicador ligado à ATIVIDADE, que é a forma do problema que criou o REQ-032.
  > ⚠️ Em modo de DESENVOLVIMENTO o Next desliga o prefetch, então este risco **não se mede em dev** — foi preciso `next start` sobre um build de produção. Entrada `prod-local` no `.claude/launch.json`.
...
- [x] **TASK-098 → tutorial de primeiro acesso, pt-BR, por papel.** ✅ **FEITA em 2026-09-10 — fecha o ADR-018 em código.** 4 passos para quem opera o balcão (com a dupla confirmação explicada, que é a regra que mais gera dúvida), 3 para quem só usa. Dispensável por Escape, botão ou clique fora, e sair já conta como visto. Coluna `onboarding_visto_em` (timestamp, não booleano) com `COALESCE` preservando a primeira data. A rota `POST /api/account/onboarding` **não recebe corpo** — a assinatura é a garantia contra marcar linha alheia. Verificado no navegador nos dois papéis e nos dois caminhos. (Tipo A — feature nova.) PORTEIRO precisa aprender o balcão; ALUNO precisa saber onde vê as próprias chaves e como confirmar — o mesmo tutorial para os dois ensina a pessoa errada. O "já viu" vai em **coluna de `users`, não `localStorage`**: a pessoa é a mesma em qualquer aparelho, e no computador compartilhado do balcão o `localStorage` erra nos dois sentidos. Pulável, navegável por teclado, e não pode bloquear a primeira retirada de chave.
...
- [x] **TASK-094 → a senha compartilhada sai.** ✅ **FEITA em 2026-09-09.** Sai de tudo: `GET /api/settings` devolve UM campo, o POST não grava, `SENHA_PADRAO_RESET` morreu, o campo da tela virou explicação de onde o acesso é recuperado, e o contrato registra a remoção. Migration `202609090900` desarma quem tinha a senha antiga — e a condição é o coração dela: `requires_password_change` sozinho derrubaria o CÓDIGO de quem está a caminho do balcão. Três testes antigos cederam e nenhum foi silenciado; um cenário da TASK-087 (o `GET` verifica a sessão por conta própria, §3.2) foi RESGATADO para o arquivo novo em vez de morrer com o antigo. `settings.default_reset_password`, o campo da tela de Configurações, `SENHA_PADRAO_RESET`, o `defaultResetPassword` do `GET`/`POST /api/settings` e a entrada correspondente em `docs/api-contract.md`.
...
- ✅ ~~🔴 **VAZAMENTO DE WEBSOCKET a cada navegação**~~ — **CORRIGIDO em 2026-09-09 (TASK-105).** Cliente único por aba, criado preguiçosamente. Recontado no navegador com as MESMAS três navegações: **4 sockets criados / 4 abertos → 0 / 0**, e o card de tempo real continua "Ativa". Registro original abaixo, porque o número é o que dá sentido à correção.
