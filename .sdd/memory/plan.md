...
- **Erro de hidratação pré-existente** — `src/app/history/HistoryClient.tsx:183` renderiza `new Date().toLocaleString('pt-BR')` direto no JSX: o servidor marca um instante no SSR e o cliente outro na hidratação, e o React descarta a árvore renderizada. Vem do commit original `95af5ac`, e o mesmo sintoma aparece no Dashboard. Detectado ao verificar a Sprint 17 no navegador; fora do escopo daquela sprint por ser padrão sistêmico, não local. Correção: renderizar em `useEffect` ou marcar `suppressHydrationWarning` no nó.
