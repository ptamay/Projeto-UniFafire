...
- ✅ #59 merged (CR). ✅ **TASK-111 FEITA** (branch `feat/task-111-tela`, PR a abrir): "?" no shell
  (desktop + celular), tempo real como ponto, backup só com o estado. Verificado no navegador.
- ⚠️ **COMO VERIFICAR TELA AUTENTICADA SEM DIGITAR SENHA:** assinar o JWT com o `JWT_SECRET`
  carregado pelo `@next/env` (`loadEnvConfig` — ler o `.env.local` à mão deu segredo DIFERENTE),
...
- ✅ #76 merged (TASK-116 no ar — ADR-024 fechado em código). 📋 **CR da trilha imutável (Tipo C,
  ADR-026), aprovado pelo usuário:** MEDIDO — `UPDATE`/`DELETE` passam em `action_logs` e
...
- ✅ #80 merged (TASK-124 NO AR, `7ef0c5d`): roteiro aplicado pelo usuário ANTES do merge (14 linhas
  ...
  (A queixa da `higiene-de-carga` com o `@vercel/speed-insights` não commitado no checkout principal
  some com o #81: o pacote entrou junto com o importador.)
...
- Próxima Ação: merge do CR do PWA → TASK-129. Do usuário: conferir em produção a parte logada da TASK-125 e,
...
- 📋 **CR da TROCA DE TELA SUAVE (Tipo C, ADR-029), aprovado pelo usuário, 2026-09-14:** depois da
  ...
  ✅ #90 (CR) aberto. ✅ **TASK-131 FEITA** (branch `feat/task-131-dados-iniciais`): vitest 833/87, E2E 6 ok no
  build de produção, reprova contra a `main`. ✅ TASK-128 validada com LOCK em `users` (0 quadros em
  branco em 54 navegações). ⚠️ Prova de espera: LOCK na base, não `page.route` no `?_rsc=`.
...
- Branch atual: cr/pwa-sem-service-worker (CR do ADR-030, PR a abrir). PRs #15–#86 merged; aberto
  só o #43 do Dependabot.
