...
| `/api/backups/restaurar` | `GET`, `POST` | **A** | **Destrutivo (§3.5).** `GET` → `{configurado, restauraveis: [{arquivo, em, tamanho}], ocultosPorSchema, pendente, ultima}` — restaurável = verificado em `backup_runs`, dentro da retenção e posterior à última migration aplicada. `POST {arquivo, confirmacao: "RESTAURAR"}` (Zod, palavra exata) — o servidor confere que o arquivo está NA LISTA, grava `RESTAURACAO_SOLICITADA` na trilha **antes** de disparar `restaurar.yml` (`workflow_dispatch`, inputs `arquivo` e `pedido_por`) e responde **202**. **400** sem a palavra ou fora da lista; **409** com restauração pendente; **502** GitHub recusou (grava `RESTAURACAO_FALHOU`); **503** sem configuração. O workflow fecha o pedido com `BACKUP_RESTAURADO`, `RESTAURACAO_RECUSADA` ou `RESTAURACAO_FALHOU`. TASK-115 |
