...
- [x] Os 4 pares `test(TASK-NNN)` → `feat(TASK-NNN)` na ordem, com a suíte inteira verde a cada um
- [x] Migration de `app_logs` com DOWN escrito antes do UP
- [x] `./scripts/ci-gates.sh` limpo (6 gates), `tsc --noEmit` 0, `eslint` 0
- [x] `npm audit` sem HIGH/CRITICAL (constitution §6.3) — **lido inteiro, sem `head`/`tail`**
- [x] `npm run build` verde — a Sprint 21 provou que ele pega o que a suíte não pega
- [x] App exercitado no navegador contra o Postgres do container, incluindo **login com o
